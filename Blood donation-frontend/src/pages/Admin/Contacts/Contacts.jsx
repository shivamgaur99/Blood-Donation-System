...
const ContactCard = ({ contact, onDelete, onReply }) => {
  const handleDelete = () => {
    Swal.fire({
      title: 'Are you sure?',
...
const Contacts = ({ theme }) => {
  ...
  const handleReply = (contact) => {
    console.log("Replying to:", contact);
  };
  ...
  return (
    <ThemeProvider theme={theme ? darkTheme : lightTheme}>
      <div style={{ padding: "20px", backgroundColor: theme ? "#121212" : "#fff" }}>
            ...
            {contacts.length > 0 ? (
              contacts.map((contact) => (
                <Grid item xs={12} sm={6} md={4} key={contact.id}>
                  <ContactCard contact={contact} onDelete={handleDelete} onReply={handleReply} />
                </Grid>
              ))
            ) : (
